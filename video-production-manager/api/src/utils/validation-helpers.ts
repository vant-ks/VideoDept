...
export async function validateEntityExists(
  ...
  switch (entityType) {
    case 'source':
      ...
      entity = await prisma.computers.findFirst({ 
        where: { 
          id: entityId,
          is_deleted: false 
        } 
      ...
      break;
    case 'camera':
      entity = await prisma.cameras.findUnique({ where: { uuid: entityId } });
      break;
    case 'ccu':
      entity = await prisma.ccus.findUnique({ where: { uuid: entityId } });
      break;
    case 'send':
      entity = await prisma.sends.findUnique({ where: { uuid: entityId } });
      break;
    // Add more as needed
    default:
