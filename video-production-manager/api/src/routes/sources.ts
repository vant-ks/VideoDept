...
router.post('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    console.log('🔍 sourceData after destructuring:', sourceData);
    console.log('🔍 sourceData.category:', sourceData.category);
    ...
    if (outputs && outputs.length > 0) {
      ...
      for (let i = 0; i < outputs.length; i++) {
        ...
        // Get the uuid of just-created source to use as FK
        const sourceUuid = await prisma.$queryRaw`SELECT uuid FROM sources WHERE id = ${snakeCaseData.id}` as any[];
        await prisma.$executeRaw`
    ...
    const source = await prisma.$queryRaw`
      ...
      GROUP BY s.uuid, s.id, s.production_id, s.category, s.name, s.type, s.rate, 
    ...
    console.log('📝 Recording event for source:', createdSource.uuid);
    await recordEvent({
      productionId,
